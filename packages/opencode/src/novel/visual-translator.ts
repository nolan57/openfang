...
export function enrichBeatWithVisuals(beat: LiteraryBeat, characterStates: CharacterState[]): EnrichedBeat {
  const scene = {
  ...
  const visualSpec = assemblePanelSpec({
    beat,
...
// ============================================================================
// STORY TO PANELS TRANSLATION (Configuration-Driven)
// ============================================================================

/**
 * Detects action type from text using keywords from configuration.
 */
function detectActionFromConfig(text: string): string {
  const cfg = getConfig()
